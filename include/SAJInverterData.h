...
class SAJInverterData {
	...
	protected:
		...
		// Constants
		const float dcVLimits[2]  = {0.0, 1000.0};
		const float dcILimits[2]  = {0.0, 12.0};
		const float acVLimits[2]  = {210.0, 250.0};
		const float acILimits[2]  = {0.0, 50.0};

		const float busVLimits[2] = {0.0, 1000.0};
		const float tempLimits[2] = {-20.0, 100.0};

		const float PLimits[2]  = {0.0, 5000.0};
		const float QLimits[2]  = {-5000.0, 5000.0};
		const float PfLimits[2] = {-1.01, 1.01};

		const float fLimits[2]  = {45.0, 55.0}; // Nominal 50 Hz

		int   readIntU16(ModbusMessage msg, int position, float scale);
    ...
    SAJInverterData(const SAJInverterData &m);
		...
		// Multiply by scalar
		SAJInverterData scale(float scale);
